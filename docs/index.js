...
function sidebarTemplate() {
  return {
    ...
    data: [
      ...
      {divider: true}
    ].concat(Object.keys(UI.definitions).sort().map(function (n) {
    ...
    on: {
      onItemClick: function (item) {
        ...
        if (!item.link) {
          ...
          $$('methodList').parseMethods(UI.definitions[item.value]);
          $$('cssForm').parseProperties(UI.definitions[item.value]);
          $$('miscForm').parseProperties(UI.definitions[item.value]);
...
UI.new({
  ...
  layout: 'column',
  hidden: true,
  visibleBatches: ['properties', 'tab', 'component'], // Custom field
  cells: [
    {
      batch: 'tab',
      view: 'list',
      ...
      on: {
        onItemClick: function (item) {
          ...
          var batches = mainView.visibleBatches;
          switch (item.value) {
            case 'component':
              batches.splice(batches.indexOf('code'), 1, 'component');
              mainView.showBatch(mainView.visibleBatches);
              break;

            case 'code':
              batches.splice(batches.indexOf('component'), 1, 'code');
              mainView.showBatch(mainView.visibleBatches);
    ...
    },
    {
      ...
      parseCode: function (name) {
        ...
        var defaults = UI.definitions[name].prototype.$defaults;
    ...
    },
    {
      ...
      on: {
        onItemClick: function (item) {
          ...
          switch (item.value) {
            case 'properties':
              batches.splice(batches.indexOf('methods'), 1, 'properties');
              mainView.showBatch(mainView.visibleBatches);
              break;

            case 'methods':
              batches.splice(batches.indexOf('properties'), 1, 'methods');
              mainView.showBatch(mainView.visibleBatches);
              break;
          }
        }
                ...
                dropdown: {
                  ...
                  on: {
                    ...
                    onItemClick: function (item) {
                      ...
                      this.masterComponent.config.label = UI.interpolate(
